...
2-2-1 解锁的手机&备份好手机数据
2-2-2 PC
2-2-3 ADB工具&Fastboot工具
2-2-4 第三方的Recovery TWRP
2-2-5 第三方ROM Lineage OS & ROOT包(可选)

3-3 大致的流程

 3-3-1 手机开机状态(打开USB调试),用ADB刷入TWRP-Recovery 或者手机fastboot状态(mi4-开机键+音量-)下,用Fastboot刷入TWRP-Recovery.
   
3-3-2 手机重启时进入TWRP-Recovery(mi4-开机键=音量+)或者手机重启后用ADB命令.
   
 `adb reboot recovery`

3-3-3 手机在TWRP-Recovery下双清wipe,之后用ADB命令写入卡刷包.  

 `adb push filename.zip /sdcard/`

 p.s.其实TWRP可以识别OTG的U盘,可以先把相关文件(recovery(*.img)、卡刷包(*.zip)、Root包(*.zip))前提U盘的读写速度可以，想想有些刷机包1点多G的...233

3-3-4 手机在TWRP-Recovery下install,刷入卡刷包，这时候也是可以继续刷入opengapps、root包的.
    
3-3-5 最后就可以重启手机了，开始新生！
...
12-12 开始写入卡刷包到手机内置sdcard (DATA分区)
...
13-13 返回主界面,开始在TWRP上刷入Lineage OS
